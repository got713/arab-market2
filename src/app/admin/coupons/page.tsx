    ...
    <div className="space-y-6 fade-in">
      ...
      <div className="flex justify-between items-center border-b border-light-border pb-4">
        <div className="text-xs text-gray-500 font-medium">
          Active promotions: <strong>{coupons.length} coupon codes</strong>
        </div>
        <button
                ...
                {coupons.map((c) => {
                  ...
                  return (
                    <tr key={c.code} className="hover:bg-cream/10 transition-colors">
                      ...
                      {/* Percent */}
                      <td className="p-4 font-bold text-dark text-sm">{c.discountPercent}% OFF</td>

                      {/* Min order */}
                      <td className="p-4 font-semibold text-gray-600">${c.minOrder.toFixed(2)}</td>

                      {/* Usage */}
                      <td className="p-4 font-semibold text-dark">
                        {c.usageCount} / {c.maxUsage} uses
                      </td>
          ...
          <div className="relative w-full max-w-md bg-white rounded-xl shadow-2xl border border-light-border overflow-hidden">
            ...
            <form onSubmit={handleFormSubmit} className="p-6 space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase">Discount Percent (%) *</label>
                  <input
                    type="number"
                    required
                    min={1}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase">Minimum Order ($) *</label>
                  <input
                    type="number"
                    required
                    min={0}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase">Usage Quota Limit *</label>
                  <input
                    type="number"
                    required
                    min={1}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase">Expires (YYYY-MM-DD) *</label>
                  <input
                    type="date"
                    required
                    value={expires}
